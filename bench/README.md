...
survey recorded with the Havern County Auditor under 201907220018. A recorded
...
- `ocr-esize-survey` — the 27 x 36.7 inch record of survey under 201503110045.
...
## First run (2026-08-02, Qwen3-6-27B-MPT, warm)
    ...
    ocr-survey-facts   14738 prompt tokens, 53s, 334 tok/s   5/7 checks
      ...
      FAIL  201907220016     note 2's auditor file number
      PASS  201503110045     the deed's auditor file number
      PASS  201907220018     the recording number
      FAIL  DACE           read as DACF
      PASS  not 2015031104   the misreading a person corrected by hand
      PASS  not 201364       what this model returned at 200 DPI

That surveyor-name row was recorded as `DACE` / read as `DACF`, and both were
...
`DACE` to `DACF`, which is the failure the check is for.
