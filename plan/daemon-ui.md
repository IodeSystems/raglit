...
- **`page-unread` rendered NOWHERE.** `Health.tsx` filtered problems through a
  ...
  name the one that reports the salvage holes. Two sat in the fence-dispute index and
...
- Verified live on the fence-dispute index after `go install` + `systemctl --user
...
I typed `sqlc generate`, got `$HOME/go/bin/sqlc` (upstream v1.30.0), and
...
### ⏸ the model gateway was returning 503 for everything, 2026-08-11
...
`Order on Motion to Continue.docx` in the fence-dispute evidence folder failed as
