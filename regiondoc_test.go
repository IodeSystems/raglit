	...
	root := &Region{
		...
		Children: []*Region{
			...
			{Page: 1, BBox: Rect{0.6, 0.7, 0.3, 0.2}, Depth: 1, Kind: "title-block",
				Text: "AF#9404150061", DPI: 200, SHA256: "cc", TokensPerSqIn: 44},
		},
	}
	root.assignIDs("p1")
	return root
...
func TestRegionTranscriptKeepsOverviewsAndLeaves(t *testing.T) {
	...
	for _, want := range []string{"record of survey", "WESTERLY", "AF#9404150061"} {
...
func TestAQuoteInBothAnOverviewAndALeafPrefersTheDeeperRead(t *testing.T) {
	root := attestTree()
	root.Text = "A record of survey. AF#9404150061 appears in the title block."
	p := RegionPage{Page: 1, Root: root}
	hits := p.RegionsForQuote("AF#9404150061")
	if len(hits) != 2 {
