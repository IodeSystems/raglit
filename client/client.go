...
// DocSummary is one document as the INDEX holds it: what it is, and how much of
// it was actually read.
//
// Fragments is the number that answers "does this have readable text". A row
// with zero is IN the index and was never read — a scan nobody OCR'd, an image
// nobody captioned — and that is a different state from a document the index has
// never heard of, which is a different state again from one on disk with a text
// layer. Collapsing any two of them produces a backlog that asks somebody to
// classify a document nothing can read.
type DocSummary struct {
	Index string `json:"index"`
	Path  string `json:"path"`
	Title string `json:"title,omitempty"`
	// Fragments is the indexed text: zero means nothing readable came out of it.
	Fragments int `json:"fragments"`
	// Pages is the page COUNT, not the pages. A document may have pages and no
	// fragments, which is exactly the un-OCR'd scan.
	Pages int `json:"pages"`
	// Vision counts what was read by a vision model rather than a text layer.
...
//
// IT DECODED INTO THE WRONG TYPE AND THEREFORE ALWAYS FAILED. This returned
// []Doc, whose `pages` is a LIST of pages with their text; the endpoint sends
// `pages` as a COUNT. Every real response failed to unmarshal — "cannot
// unmarshal number into Go struct field Doc.documents.pages" — so this function
// had never once returned a document.
//
// The consequence was silent and lived one layer up: kgraph's `docsWithText`
// maps an error to nil, meaning "could not ask", and its caller falls back to
// looking for a sidecar on disk. So the backlog was permanently answering the
// question the index was supposed to have answered, for every corpus, and the
// fallback made it look like it was working.
func (c *Client) Documents(ctx context.Context, index string) ([]DocSummary, error) {
	v := url.Values{}
	if index != "" {
		v.Set("index", index)
	}
...
// Withdrawal is a document ruled OUT of the corpus, with the grounds recorded.
//
// Withdrawing and deleting are different acts, and the difference is the whole
// point of reading this: a delete removes rows and the next sweep puts the
// document back, so it is a preference. A withdrawal is a RULING — it carries
// grounds, it is recorded where git can blame it, and the ingest path honours
// it. A consumer that walks the directory instead of asking will offer a
// withdrawn document as a candidate forever, identically on every run, and
// nothing anywhere will say it was ruled out.
//
// Only Path and Reason: the report this reads is keyed on the ruling, and who
// ruled and when are not on it. Ask the judgement store if you need them.
type Withdrawal struct {
	Path   string `json:"path"`
	Reason string `json:"reason"`
}

// Withdrawals lists what has been ruled out of an index, path order.
//
// Read off the health report rather than a ledger endpoint of its own, because
// that report is already where a withdrawal is published — `kind=withdrawn` IS
// the list, and it is exempted there from the filter that hides withdrawn
// documents from every other kind.
//
// AN UNREACHABLE DAEMON IS AN ERROR, and a caller must not read it as "nothing
// has been withdrawn". Those are the two answers this exists to keep apart: one
// says a ruling was made, the other says nobody could ask. `get` returns
// ErrUnavailable for both a stopped daemon and one too old to have the route.
func (c *Client) Withdrawals(ctx context.Context, index string) ([]Withdrawal, error) {
	v := url.Values{"kind": {"withdrawn"}}
	if index != "" {
		v.Set("index", index)
	}
	var out struct {
	...
	if err := c.get(ctx, "/api/problems", v, &out); err != nil {
		return nil, err
	}
	ws := make([]Withdrawal, 0, len(out.Problems))
	for _, p := range out.Problems {
		ws = append(ws, Withdrawal{Path: p.Subject, Reason: p.Detail})
	}
	return ws, nil
}
