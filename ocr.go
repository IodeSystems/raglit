...
type OCR struct {
	...
	// vision model normalises proper names (DACE → DACF) and tesseract does
...
func (o *OCR) visionPage(ctx context.Context, img PageImage, assist string) (string, int, error) {
	...
	if rep != nil {
		...
		// filled row (Cartwright → Quill, 1972). Both looped on
		...
		if rep != nil {
			...
			// which is exactly how page 9 of the dace exhibit failed, on a
		...
		// plausible auditor file numbers (A#201404150061 for AF#9404150061). It
