...
const oidioBody = `{
  ...
  "segments": [
    {"id":0,"start":1.5,"end":12.25,"text":"I went over there.","speaker":"uuid-milo"},
    {"id":1,"start":12.25,"end":30.0,"text":"That is not what happened.","speaker":"uuid-tess","overlap":true},
    {"id":2,"start":30.0,"end":31.0,"text":"  ","speaker":"uuid-milo"}
  ],
  "speakers": [
    {"uuid":"uuid-tess","total_seconds":17.75,"clean_seconds":17.75},
    {"uuid":"uuid-milo","total_seconds":11.75,"clean_seconds":2.0,"blended":true}
  ]
}`
...
func TestSTT_UploadsOriginalBytesAndAsksForStructure(t *testing.T) {
	...
	if _, err := s.Transcribe(context.Background(), "/corpus/H19-440-hearing.mp4", raw); err != nil {
		t.Fatal(err)
	}
	...
	if g.Filename != "H19-440-hearing.mp4" {
		t.Errorf("filename = %q, want H19-440-hearing.mp4", g.Filename)
	}
}
...
// transcript reading "uuid-milo: I went over there" is not a transcript.
func TestSpeakerLabels_OrderedBySpeakingTimeNotMapOrder(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		if l["uuid-tess"] != "speaker-1" || l["uuid-milo"] != "speaker-2" {
...
func TestTranscriptText_CarriesWhatASearchHitNeedsToBeTakenBack(t *testing.T) {
	...
	out := TranscriptText("/corpus/H19-440-hearing.mp4", &r)
...
func TestSTTReading_IsReviewableByAttest(t *testing.T) {
	...
	rd, err := STTReading("/corpus/H19-440-hearing.mp4", "abc123", "oidio/stt-diarize", &r)
	if err != nil {
		t.Fatal(err)
	}
	...
	if rd.Asset.ID != "/corpus/H19-440-hearing.mp4" {
...
func TestWorker_IngestsARecordingAsItsTranscript(t *testing.T) {
	...
	media := filepath.Join(corpus, "H19-440-hearing.mp4")
	...
	if !strings.HasSuffix(hits[0].Path, "H19-440-hearing.mp4") {
...
func TestHome_TranscriptStaysOutOfTheCorpus(t *testing.T) {
	h := Home(t.TempDir())
	dir := h.TranscriptDir("/corpus/court/hearings/H19-440-hearing.mp4")
	...
	if dir != h.TranscriptDir("/corpus/court/hearings/H19-440-hearing.mp4") {
		t.Error("the location must be deterministic")
	}
	if dir == h.TranscriptDir("/elsewhere/H19-440-hearing.mp4") {
