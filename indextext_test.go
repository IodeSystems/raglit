...
func TestFigureDescriptionsSurvive(t *testing.T) {
	in := `<div data-label="Image"><img alt="Barcode with number 201503110046 and Havern County Auditor stamp dated 3/11/2015."/></div>`
	got := StripLayoutMarkup(in)
	if !strings.Contains(got, "201503110046") || !strings.Contains(got, "Havern County Auditor") {
...
func TestTableCellsStayOnOneLine(t *testing.T) {
	got := StripLayoutMarkup(`<tr><td>Milo Quill</td><td>04/09/2019</td></tr>`)
	first := strings.SplitN(strings.TrimSpace(got), "\n", 2)[0]
	if !strings.Contains(first, "Milo Quill") || !strings.Contains(first, "04/09/2019") {
