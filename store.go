...
func migrate(db *sql.DB) error {
	cols := []struct{ table, col, def string }{
		...
		{"documents", "gen_content_tags", "TEXT NOT NULL DEFAULT ''"},
...
func (s *Store) Ingest(ctx context.Context, doc Document) error {
	...
	for _, f := range doc.Fragments {
		...
		// A PAGE WE COULD NOT READ IS NOT A PAGE THAT SAYS NOTHING, and storing the
		// bytes conflates them. A failed image extraction hands back raw JPEG, and
		// writing that into `fragments` makes it searchable text: it will never
		// match a real quotation, so a citation to that page reports ABSENT — "the
		// document does not say this" — when the truth is that nobody could read
		// it. Downstream models that distinction (kgraph reports `unreadable`
		// separately for exactly this reason) and cannot apply it to bytes we
		// claimed were text.
		//
		// Found in a live corpus: a 2008 survey supporting six facts had page 1
		// stored as `(\xff\xd8 (\xff\xd8 …`, and a query over the index crashed
		// decoding it.
		if !isReadableText(f.Text) {
			unreadable++
			continue
		}
	...
	if unreadable > 0 {
		// REPORTED, not silent. Skipping is the right storage decision and a
		// silent skip is not: a document that lost half its pages to a failed
		// extraction looks identical to a short one.
		fmt.Fprintf(os.Stderr, "raglit: %d page(s) of %s produced bytes rather than text "+
			"and were NOT stored — the pages are unreadable, not empty; re-index to retry "+
			"the extraction\n", unreadable, doc.Path)
	}
...
// isReadableText reports whether an extracted fragment is text at all.
//
// Two failures produce the same symptom — bytes where a transcription should be:
// an image extractor handing back the image, and an OCR client returning a raw
// response body on an error path. Both are caught here rather than at each call
// site, because the point is that nothing downstream should have to guess.
//
// Deliberately permissive about CONTENT: a survey is mostly numbers, a plat is
// mostly punctuation, and a foreign-language exhibit is mostly non-ASCII. What is
// rejected is text that is not valid UTF-8, or that is mostly control bytes —
// neither of which any transcription of any document should be.
func isReadableText(s string) bool {
	...
	// A tenth is far past anything a real document produces and far below what a
	// binary blob does, so the threshold does not have to be argued over.
	return ctrl*10 < total
}
