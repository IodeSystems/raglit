...
func TestSegmentShortDetectsADroppedPage(t *testing.T) {
	...
	var got []stagedFrag
	_, err := segmentLLMWith(context.Background(), drops("EXISTING CORNERS"), pages, nil, nil,
...
func TestSegmentShortToleratesReflow(t *testing.T) {
	page := strings.Repeat("Lot I of survey recorded under recording number 201503110045. ", 10)
...
func TestSegmentShortIgnoresTinyPages(t *testing.T) {
	...
	_, err := segmentLLMWith(context.Background(), drops("p. 3"), pages, nil, nil, func(stagedFrag) {})
	if err != nil {
