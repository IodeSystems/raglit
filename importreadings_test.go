...
func TestImportVerifiedTranscripts_AdoptsAndCollapses(t *testing.T) {
	s := testStore(t)
	const machine = "speaker-1 calling first the matter of bramble versus quill " +
		"speaker-2 the respondent has the burden to show a substantial change in circumstances"
	const verified = "Clerk calling first the matter of Bramble versus Quill " +
		"Milo the respondent has the burden to show a substantial change in circumstances"
...
func TestImportVerifiedTranscripts_ASelfReadingDoesNotBlockAdoption(t *testing.T) {
	...
	rec := seedRecording(t, s, dir, "h19-65.mp4", "sha-6565", machine)
	tp := writeTranscript(t, dir, "h19-65.verified.md",
