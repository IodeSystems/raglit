...
func TestReadingsAccumulateAndTheOldOneIsKept(t *testing.T) {
	...
	if err := js.PutPageCorrection(PageCorrection{
		Doc: doc, Page: 1, Text: "AF 201503110045 MARA G. DACE",
		Supersedes: "AF 2015031104 DACF",
	...
	if got[0].Text != "AF 2015031104 DACF" {
	...
	if err := js.PutPageCorrection(PageCorrection{
		Doc: doc, Page: 1, Text: "AF 201503110045 MARA G. DACE, cert 20123169", By: "carl",
	}); err != nil {
		t.Fatal(err)
	}
...
func TestPageReadings_SourcedAndAPersonHoldsTheSeat(t *testing.T) {
	...
	if err := s.AddPageReading(ctx, PageReading{Doc: doc, Page: 1,
		Text: "CERTIFICATE NO. 2015031104", Source: "machine",
		Engine: "vision", Model: "Qwen3-6-27B-MPT"}); err != nil {
		t.Fatal(err)
	}
	if err := s.AddPageReading(ctx, PageReading{Doc: doc, Page: 1,
		Text: "CERTIFICATE NO. 201503110045", Source: "corrected",
	...
	if err := s.AddPageReading(ctx, PageReading{Doc: doc, Page: 1,
		Text: "CERTIFICATE NO. 20150311", Source: "machine",
	...
	if err := s.Ingest(ctx, Document{Path: doc, Title: "ros.pdf",
		Fragments: []Fragment{{Page: 1, Ord: 0, Text: "CERTIFICATE NO. 2015031104"}}}); err != nil {
		t.Fatal(err)
	}
	txt, err := s.IdentityText(ctx, doc)
	if err != nil || !strings.Contains(txt, "201503110045") {
